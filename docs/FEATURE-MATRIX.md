...
| Dynamic mode lists: grow a LIVE monitor's list, no destroy/create cycle (frame generation enabled mid-stream) | New (build 17, DESIGN.md §3.2.5) | **Implemented** (`UPDATE_MODES`, proto 0.5, `caps::DYNAMIC_MODES`, `IddCxMonitorUpdateModes2`); additive-only. Whether the OS re-parses so an ADDED mode surfaces is unverified — see §3.2.5 |
