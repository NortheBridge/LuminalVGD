...
| Dynamic mode lists: re-publish which of a LIVE monitor's create-time modes it offers, no destroy/create cycle (frame generation enabled mid-stream) | New (build 17, DESIGN.md §3.2.5) | **Implemented** (`UPDATE_MODES`, proto 0.5, `caps::DYNAMIC_MODES`, `IddCxMonitorUpdateModes2`); replaces the TARGET subset within the create-time superset — it cannot ADD a mode, and no IddCx 1.10/1.11 DDI can. Whether the DDI replaces or appends the target list is unverified (safe either way) — see §3.2.5 |
